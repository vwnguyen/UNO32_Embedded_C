...
// HELPER FUNCTION;
// newNode creates a new node given the char*
...
// freeNode()
// destroys nodes and sets their pointers to NULL
...
Node *TreeCreate(int level, const char *data) {
    ...
    int howManyElements = pow(2, level);
    howManyElements = howManyElements - 1;
    if (level == 0) {
        return NULL;
    }
    int newInd = howManyElements / 2;
    level = level - 1;
    ROOT->leftChild = TreeCreate(level, data + 1);
    ROOT->rightChild = TreeCreate(level, data + 1 + newInd);
    return ROOT;
}
...
void PrintTree(Node *node, int space) {
    // return nothing if NULL to last recursive call
    if (node == NULL) {
        return;
    }
    int i;
    // increase the space count for each call
    space = space + LEVEL_SPACES;
    PrintTree(node->rightChild, space);
    // print the current node after the spaces
    // print newline first, 
    printf("\n");
    for (i = LEVEL_SPACES; i < space; i++) {
        printf(" ");
    }
    // if data is not printable 
    printf("%c\n", node->data);
